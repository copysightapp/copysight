...
for (const html of [english, spanish]) {
  ...
  assert.doesNotMatch(html, /\/downloads\/|Direct download|Descarga directa/i);
  assert.equal((html.match(/https:\/\/github\.com\/copysightapp\/copysight/g) ?? []).length, 3);
  assert.match(html, /https:\/\/apps\.apple\.com\/app\/id6797554906/g);
  assert.match(html, /⌃ ⌘ 2/);
  assert.match(html, /application\/ld\+json/);
  assert.match(html, /hreflang="en"/);
  assert.match(html, /hreflang="es"/);
  assert.match(html, /open-graph\.png/);
  assert.match(html, /twitter:card/);
  assert.match(html, /"softwareVersion": "1\.1\.1"/);
  assert.match(html, /"price": "0\.99"/);
}
...
assert.equal(config.headers.some(({ source }) => source === "/downloads/(.*)"), false);
